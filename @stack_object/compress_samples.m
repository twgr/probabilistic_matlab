function samples = compress_samples(samples, T)
%compress_samples
%
...
b_zero_weight = samples.relative_particle_weights==0;
if all(b_zero_weight)
    samples.relative_particle_weights = 1e-100*ones(size(samples.relative_particle_weights));
else
    min_non_zero_weight = min(samples.relative_particle_weights(~b_zero_weight));
    ...
    samples.relative_particle_weights(b_zero_weight) = weight_to_set_zeros_to;
end
...
for n_f = 1:numel(p_fields)
    assert(isnumeric(samples.var.(p_fields{n_f})),'Currently only supports compression when all variables are numeric');
    variable_sizes(n_f,:) = size(samples.var.(p_fields{n_f}));
    bContinuous(n_f) = rem(samples.var.(p_fields{n_f})(1),1)~=0;
...
if T==1 && isempty(iDiscrete)
    ...
    samples.relative_particle_weights = samples.relative_particle_weights;
    return
end
...
if (all(bFullWidth) && any(bContinuous)) || (numel(bFullWidth)==1)
    ...
    [samples.var.(p_fields{var_to_use}),i_row_sort] = sortrows(samples.var.(p_fields{var_to_use}));
    w = samples.relative_particle_weights(i_row_sort);
    samples.relative_particle_weights = w;
    [sX1,sX2] = size(samples.var.(p_fields{var_to_use}));
    bUnique = [true(1,sX2);diff(samples.var.(p_fields{var_to_use}),[],1)~=0];
    ...
    for n_f = 1:numel(p_fields)
        vars_this = samples.var.(p_fields{n_f})(indsU);
        b_zero = vars_this==0;
        if any(b_zero)
        ...
        samples.var.(p_fields{n_f}) = sparse(iU,jU,vars_this,sX1,sX2);
    end
    try
    ...
    samples.sparse_variable_relative_weights = w;
else
    ...
    for n_f = 1:numel(p_fields)
        all_variables = [all_variables,samples.var.(p_fields{n_f})]; %#ok<AGROW>
        samples.var.(p_fields{n_f}) = [];
    end
    ...
    for n_f = 1:numel(p_fields)
        ...
        samples.var.(p_fields{n_f}) = sparse(iU(bThis),jU(bThis),vars_this,sX1,variable_sizes(n_f,2));
        i_iU1 = iU(bThis)==1;
        ...
        samples.sparse_variable_relative_weights.(p_fields{n_f}) = sparse(iU(bThis),jU(bThis),weights_this,sX1,variable_sizes(n_f,2));
